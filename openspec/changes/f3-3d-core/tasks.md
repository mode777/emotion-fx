...
- [x] 10.3 Dispatch `gh workflow run ci.yml --ref <branch>`; the full four-target gate passes in Linux → Windows → macOS order (native suites incl. goldens on each, Emscripten suite + web goldens), per the rendering-milestone gate; verify via the Actions run summary (green: run 36122872839 after the depth-remap fix — all six jobs success, macOS/Windows 75/75 incl. the six 3D goldens)
